...
class Menu:
    ...
    def click_button(self, pos):
        ...
        for (i, rect) in enumerate(self.erasers_rect):
            if rect.collidepoint(pos):
                #这里解决了两个BUG，一个是点击橡皮擦后点画笔丢失之前的画笔颜色
                #第二是点击橡皮擦多次再点回画笔也会丢失其颜色
                if(self.brush.get_color() == self.eraser_color):
                    pass 
                else:                   
                    self.beforecolor=self.brush.get_color()
                    self.brush.set_color(self.eraser_color)
                    return True
        # 点击画笔事件
        for (i, rect) in enumerate(self.pens_rect):
            if rect.collidepoint(pos):
                self.brush.set_color(self.beforecolor)
                return True
